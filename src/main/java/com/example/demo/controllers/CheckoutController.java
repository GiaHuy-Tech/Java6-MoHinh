...
public class CheckoutController {
	...
	@Autowired
	private AccountRepository accountRepo; // ✅ Cần repo để update user
	@Autowired
	private MembershipService membershipService; // ✅ Inject Service xử lý hạng
	@Autowired
	private AddressRepository addressRepo;

	// --- CẤU HÌNH TỈNH THÀNH ---
	private static final List<String> SOUTH_PROVINCES = Arrays.asList("ho chi minh", "hcm", "sai gon", "can tho",
			"binh duong", "dong nai", "long an", "tien giang", "ben tre", "tra vinh", "vinh long", "dong thap",
			"an giang", "kien giang", "hau giang", "soc trang", "bac lieu", "ca mau", "ba ria", "vung tau", "tay ninh",
			"binh phuoc");
	...
	public String viewCheckout(Model model) {
		...
		// lấy địa chỉ cho tính phí ship
		Address defaultAddress = addressRepo.findByAccountIdAndIsDefaultTrue(account.getId());
	...
	@PostMapping
	public String processCheckout(@RequestParam("address") String address, @RequestParam("phone") String phone,
		...
		if ("COD".equals(paymentMethod)) {
			// Đối với COD, giả định đơn thành công thì cộng điểm luôn (hoặc chờ admin
			// duyệt)
